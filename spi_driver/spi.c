...
void spi2_init(void)
{
    SPI2CON1bits.MSTEN          = 0; // set slave mode
    SPI2CON1bits.CKE            = 0; // Set clock edge select (0 = data changes on clock transition from active to idle)
    ...
    __attribute__((unused)) uint8_t dummy = SPI2BUF; // Clear the SPI2BUF register to avoid overflow
    //also, supress unused variable warning
